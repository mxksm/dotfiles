...
\`\`\``,isLoading:!r,actions:(0,w.jsx)(c.ActionPanel,{children:(0,w.jsx)(c.Action.CopyToClipboard,{content:t,shortcut:{modifiers:["cmd"],key:"c"}})})})},jn=e=>{let n=`
  ...
  `;N(t)};function Vn(e){let[t,n]=(0,I.useState)(""),[r,o]=(0,I.useState)(),[s,i]=Be("recently-used",[]),a=(0,I.useRef)(null),l=Q.default.existsSync("/Applications/iTerm.app"),u=Q.default.existsSync("/Applications/kitty.app"),y=Q.default.existsSync("/Applications/Warp.app"),g=Q.default.existsSync("/Applications/Ghostty.app"),m=p=>{i(f=>f.find(P=>P===p)?f:[p,...f].slice(0,10))};(0,I.useEffect)(()=>{try{let p=Qe().reverse(),f=Array.from(new Set(p)).filter(P=>P.trim().length>0);o(f)}catch(p){console.error("Failed to load shell history",p),o([])}},[]);let{arguments_terminal_type:E,arguments_terminal:_}=(0,c.getPreferenceValues)(),S=E?.toLowerCase?.()??(A?"powershell":"terminal"),C=()=>{if(A)return hs(S);switch(S){case"iterm":return"iTerm";case"kitty":return"Kitty";case"warp":return"Warp";case"ghostty":return"Ghostty";default:return"Terminal"}},k=p=>{if(A){ms(S)(p);return}switch(S){case"kitty":jn(p);break;case"iterm":Wn(p);break;case"warp":Hn(p);break;case"ghostty":zn(p);break;default:Xn(p);break}},b=(p,f)=>{(0,c.closeMainWindow)(),(0,c.popToRoot)(),m(p),f(p)};if((0,I.useEffect)(()=>{if(!e.arguments?.command||!_)return;let p=e.arguments.command,f=`${S}:${p}`;a.current!==f&&(a.current=f,m(p),(0,c.showHUD)(`Ran command in ${C()}`),k(p),(0,c.closeMainWindow)(),(0,c.popToRoot)())},[e.arguments?.command,_,S]),e.arguments?.command)return _?null:(0,w.jsx)(Un,{cmd:e.arguments.command});let L=[];return t&&L.push({category:"New command",items:[t]}),s.length>0&&L.push({category:"Raycast History",items:s.filter(p=>p.includes(t)).slice(0,50)}),r!==void 0&&r.length>0&&L.push({category:"Shell History",items:r.filter(p=>p.includes(t)).slice(0,50)}),(0,w.jsx)(c.List,{isLoading:r===void 0,onSearchTextChange:n,searchText:t,navigationTitle:"Shell command",searchBarPlaceholder:"Enter shell-command",children:L.map(p=>(0,w.jsx)(c.List.Section,{title:p.category,children:p.items.map((f,P)=>(0,w.jsx)(c.List.Item,{icon:c.Icon.Terminal,title:f,actions:(0,w.jsxs)(c.ActionPanel,{children:[(0,w.jsx)(c.Action.Push,{title:"Execute",icon:c.Icon.List,onPush:()=>m(f),target:(0,w.jsx)(Un,{cmd:f})}),s.length>=P?(0,w.jsx)(c.Action,{title:"Edit executed command",icon:c.Icon.Pencil,onAction:()=>{let R=s[P]??"";n(R??"")}}):null,!A&&(0,w.jsxs)(w.Fragment,{children:[u?(0,w.jsx)(c.Action,{title:"Execute in kitty.app",icon:{fileIcon:"/Applications/kitty.app"},onAction:()=>b(f,jn)}):null,l?(0,w.jsx)(c.Action,{title:"Execute in iTerm.app",icon:{fileIcon:"/Applications/iTerm.app"},onAction:()=>b(f,Wn)}):null,g?(0,w.jsx)(c.Action,{title:"Execute in Ghostty.app",icon:{fileIcon:"/Applications/Ghostty.app"},onAction:()=>b(f,zn)}):null,y?(0,w.jsx)(c.Action,{title:"Execute in Warp.app",icon:{fileIcon:"/Applications/Warp.app"},onAction:()=>b(f,Hn)}):null,(0,w.jsx)(c.Action,{title:"Execute in Terminal.app",icon:{fileIcon:"/System/Applications/Utilities/Terminal.app"},onAction:()=>b(f,Xn)})]}),A&&fs.map(R=>{let He=Y[R];return(0,w.jsx)(c.Action,{title:`Execute in ${He.label}`,icon:c.Icon.Terminal,onAction:()=>b(f,He.runner)},R)}),(0,w.jsx)(c.Action.CopyToClipboard,{title:"Copy to Clipboard",content:f,onCopy:()=>{m(f)}})]})},P))},p.category))})}0&&(module.exports={getCachedEnv});
